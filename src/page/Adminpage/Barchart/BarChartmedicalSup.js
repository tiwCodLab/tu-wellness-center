    ...
    const fetchData = async () => {
      try {
        ...
        const groupedData = filteredData.reduce((acc, item) => {
          // ตรวจสอบว่า medical_supplies มีข้อมูลหรือไม่
          if (item.medical_supplies && Array.isArray(item.medical_supplies)) {
            item.medical_supplies.forEach((medication) => {
              const medicalName = medication.supplies_name;
...
export default BarChartmedicationSup;
